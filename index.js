...
function processSum(numberList, callback) {
  ...
  return callback(numberList.reduce(add,0))
}
...
function getFullNames(runners) {
  ...
  runners.forEach(element =>{
    runner.push(`${element.last_name}, ${element.first_name}`)
      })
  return runner
}
...
function firstNamesAllCaps(runners) {
  let name = []
  runners.forEach(element =>{
  name.push(element.first_name.toUpperCase())
      })
  return name
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  /* CODE HERE */
  function equalShirt(runner) {
    return runner.shirt_size === tShirtSize;
  }
  const newArray = runners.filter(equalShirt)
  return newArray;
}
